...
class MyWidget(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        cur = self.con.cursor()
        self.name.setText(cur.execute(
            """SELECT name FROM Users WHERE active = 'True'""").fetchone()[0])
        self.surname.setText(cur.execute(
            """SELECT surname FROM Users WHERE active = 'True'""").fetchone()[0])
        self.status.setText(cur.execute(
            """SELECT status FROM Users WHERE active = 'True'""").fetchone()[0])
        self.sity.setText(cur.execute(
            """SELECT sity FROM Users WHERE active = 'True'""").fetchone()[0])
        index = ["Мужской", "Женский"]
        self.gender.setCurrentIndex(index.index(cur.execute(
    ...
    def photo_save(self):
        ...
        cur.execute(
            "UPDATE Users SET photo_active = 'True' WHERE active = 'True'""")
        self.con.commit()
